...
require_once ('conexion.php');
	...
	$newpass=$_POST['contranueva'];
		...
		if (!$dblink) {
			...
			$sqlquery->execute(array($_SESSION['usuarioSesion']));
			$result=$sqlquery->fetch();
			if (password_verify($oldpass, $result[2]))
			{
				$hashpass=password_hash($newpass, PASSWORD_BCRYPT, ['cost' => 12]);
				$update=$dblink->prepare("UPDATE usuarios SET Contrasenia=? WHERE Usuario=?");
				$update->execute(array($hashpass,$_SESSION['usuarioSesion']));
